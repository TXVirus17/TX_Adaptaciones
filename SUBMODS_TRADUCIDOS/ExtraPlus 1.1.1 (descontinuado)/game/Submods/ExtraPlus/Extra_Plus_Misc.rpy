...
label aff_log:
    ...
    if os.path.isfile(renpy.config.basedir + '/game/Submods/ExtraPlus/submod_assets/Pictograms.ttf'):
        "El afecto de [m_name] es [int(mas_affection._get_aff())] {size=+5}{color=#FFFFFF}{font=[Pictograms_font]}7{/font}{/color}{/size}"
    else:
        "El afecto de [m_name] es [int(mas_affection._get_aff())]"
    window hide
...
label mas_backup:
    show monika 1hua at t11
    m 1hub "Me alegro de que quieras hacer una copia de seguridad!"
    m 3eub "Te abriré el camino."
    m 1dsa "Dame un momento.{w=0.3}.{w=0.3}.{w=0.3}{nw}"
    window hide
    python:
...
label mas_backup_fail:
    m 1lkb "Lo siento, no puedo abrir la carpeta."
    m 1eka "Intenta de nuevo más tarde ^^."
    jump close_extraplus
    return
...
label extra_window_title:
    ...
    python:
        window_menu = [
            ("Cambiar el título de la ventana", 'extra_change_title'),
            ("Restaurar titulo de la ventana", 'extra_restore_title')
        ]

        items = [
            ("No importa", 'plus_tools', 20)
        ]
...
label extra_change_title:
    ...
    python:
        player_input = mas_input(
            prompt =("¿Qué título quieres poner en la ventana?:p"),
