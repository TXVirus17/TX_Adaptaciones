...
screen ttt_score():
    ...
    vbox:
        ...
        textbutton _("Me rindo") style "hkb_button" action [Function(ttt.set_state, -9), Function(ttt.check_state)]
        null height 6
        textbutton _("Salir") style "hkb_button" action [Hide("minigame_ttt_scr"), Jump("minigame_ttt_quit")]
...
label minigame_ttt_m_comment(id = 0):
    ...
    if id == 0:
        ...
        if moldable_variable == 0:
            m 3hua "jejeje, he ganado la partida."
            m 3hub "¡Deberias pensar en una estrategia mejor la proxima vez!"
        elif moldable_variable == 1:
            m 1sub "¡Tres en fila!"
            m 1huu "Intentalo de nuevo, jeje~"
        else:
            m 4nub "¡No te preocupes!"
            m 4hua "¡Se  que lo lograras la proxima vez!."
        #Player Wins
    elif id == 1:
        $ moldable_variable = renpy.random.randint(0, 1)
        if moldable_variable == 0:
            m 1suo "Parece que has ganado, [player]!"
            m 1suo "Pero la proxima vez hare todo lo posible por ganar,asi que preparate "
        else:
            m 1hub "Oh, esta vez ganaste...."
            m 1eub "¡Pero no tendras tanta suerte la proxima, [mas_get_player_nickname()]!"
        #Tie
    elif id == 2:
        $ moldable_variable = renpy.random.randint(0, 1)
        if moldable_variable == 0:
            m 1lkb "Oh, la pajina esta llena."
            m 1eub "!Intentemos de nuevo, [mas_get_player_nickname()]!"
        else: 
            m 3hua "No te preocupes, [player]."
            m 3hua "El plan es que nos divirtamos como pareja~"
            m 3hub "Te deseo suerte, [mas_get_player_nickname()]!"
        #Reset
    else:
        $ moldable_variable = renpy.random.randint(0, 1)
        if moldable_variable == 0:
            m 1ekd "¿Te rindes?"
            m 1eka "Esta bien, voy a reiniciar la partida, ¡Pero me llevo un punto!"
        else:
            m 1ekd "¿Que pasa, [player]?"
            m 3ekd "¿Te has distraido?"
            m 1eka "Vale, voy a reiniciar el juego, ¡Pero yo sere la ganadora de esta ronda!"
    return
...
label minigame_ttt_quit:
    ...
    if ttt.score[0] == ttt.score[1]:
        if ttt.score[0] == 0 and ttt.score[1] == 0:
            m 3esa "¡Oh! ¿dejaste de jugar?"
            m 3lkb "Pensaba que querias jugar conmigo un rato..."
            m 3lkb "Pero no te preocupes, lo entiendo si no te apetece jugar."
            m 1hua "Solo espero que podamos jugar en otra ocasion."
        else:
            m 1suo "¡Vaya, es un empate!"
            m 2huu "Aunque tiene que haber un ganador."
            m 2hub "¡La proxima vez decidiremos quien gana!"
            m 1hub "ejejeje~"
    elif ttt.score[0] > ttt.score[1]:
        m 3hua "Esta vez gane yo, [player]~"
        m 3eubsa "Pero no te sientas mal, lo que mas importa es que los dos nos divertimos."
        m 1eub "¡Quizas ganes la proxima vez!"
    elif ttt.score[0] < ttt.score[1]:
        m 1hub "Has ganado [player], Felicidades."
        m 1hub "Estoy orgullosa de ti~"
        m 3hua "¡La proxima vez tambien lo dare todo!"
