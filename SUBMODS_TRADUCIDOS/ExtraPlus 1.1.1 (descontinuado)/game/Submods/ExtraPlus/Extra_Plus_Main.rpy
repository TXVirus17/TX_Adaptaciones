#===========================================================================================
#TRADUCCION POR TX_Virus (FALTA REVISARLA ESCRITURA DE LOS BOTONES)
#===========================================================================================
# SUBMOD
#===========================================================================================
...
init -990 python in mas_submod_utils:
    Submod(
        author="ZeroFixer",
        name="Extra Plus",
        description="¡Un submod que añade un botón Extra+, además de más contenido!(traduccion por TX_Virus)",
        version="1.1.1"
...
define minigames_talk = [
    "Vamos, elige uno, [player].",
    "¿Quieres un reto nuevo?",
    ...
    "¿Jugamos a algo?",
    "Hoy me siento con suerte, [player].",
    "¿Listo para divertirte? :3",
    "¡Es hora de una competencia amistosa!",
    "¡A ver si puedes ganarme!",
    "Estoy dispuesta a jugar cualquier juego que quieras.",
]

define date_talk = [
    "¡Estoy deseando ver adónde me llevas!",
    "¿Qué aventuras nos esperan hoy?",
    "Me hace mucha ilusión pasar tiempo contigo",
    "Creemos hoy unos recuerdos inolvidables",
    "Espero que lo pasemos de maravilla juntos",
    "Vayamos donde vayamos, será perfecto siempre que estemos juntos",
    "Estoy dispuesta a cualquier sugerencia que tengas",
    "¡Hagamos algo divertido y espontáneo!",
    "¡Tengo mucha suerte de tenerte como pareja!",
    "Estoy deseando crear más recuerdos contigo hoy",
    "¡Estoy dispuesta a hacer lo que tú quieras!",
    "Hagamos de esta una cita inolvidable.",
    "¡Tengo la sensación de que hoy va a ser un día increíble!",
    "Vayamos donde vayamos, seré feliz siempre y cuando estemos juntos.",
    "¡Pasemos un día inolvidable hoy!"
]
...
define _plus_not_met = [
    ...
    "No es ese, [player].",
    "Inténtalo de nuevo~",
    "No te distraigas, [player].",
    "Sigue practicando, [player].",
    "¡Lo conseguirás la proxima vez!",
    "Más suerte la próxima, [player].",
    "¡No te rindas, [player]!",
    "Casi lo consigues, [player], ¡Sigue intentando!",
    "Esa no parece ser la correcta, [player].",
    "Lo siento, [player], no está ahí.",
    "¡Ups!, parece que te equivocaste, [player]."
]
...
init 5 python:
    ...
    def check_file_status(plus_sprites, path):
        try:
            ...
            if not set(plus_file_list) == set(plus_sprites):
                ...
                renpy.call_screen("dialog", message="Los archivos no están disponibles...", ok_action=Jump("close_extraplus"))
        except OSError as e:
            renpy.show("monika idle", at_list=[t11])
            renpy.call_screen("dialog", message="¿Por qué hay una carpeta 'game' en el submod? -.-", ok_action=Jump("close_extraplus"))   
    ...
    def draw_sprites(st, at):
        # Safe access to sprite lists - handle empty lists and invalid indices
        try:
            # Ensure we have valid integer indices
            idx_0 = persistent.chibi_accessory_layer_1 if isinstance(persistent.chibi_accessory_layer_1, int) else 0
            idx_1 = persistent.chibi_accessory_layer_2 if isinstance(persistent.chibi_accessory_layer_2, int) else 0
            
            # Get sprite names safely
            if chibi_sprites_0 and len(chibi_sprites_0) > 0:
                chibi_sprites_0_max = chibi_sprites_0[min(max(0, idx_0), len(chibi_sprites_0) - 1)]
            else:
                # Fallback to empty/transparent if no sprites available
                chibi_sprites_0_max = "0nothing"
            
            if chibi_sprites_1 and len(chibi_sprites_1) > 0:
                chibi_sprites_1_max = chibi_sprites_1[min(max(0, idx_1), len(chibi_sprites_1) - 1)]
            else:
                chibi_sprites_1_max = "0nothing"
        except Exception:
            # If anything goes wrong, use fallback values
            chibi_sprites_0_max = "0nothing"
            chibi_sprites_1_max = "0nothing"
    ...
    class extra_gift:
        ...
        def __call__(self):
            ...
            with open(file_path, 'a') as f:
                renpy.notify('¡Listo!, El archivo de [extraplus_gift[0]] se creo correctamente.')
            renpy.jump('plus_make_gift')

#====Downgrade/Migration Fix
#====Handles cases where users downgrade from newer versions (e.g., 1.4.x to 1.1.1)
init 5 python:
    def migrate_chibi_data():
        """
        Migrates chibi data from incompatible formats.
        v1.4.x uses tuples like ("darling", "idle", "blink", "hover")
        v1.1.1 uses lists like ["sticker_up", "sticker_sleep", "sticker_baka"]
        This prevents errors when downgrading.
        """
        try:
            costume = persistent.chibika_current_costume
            
            # Check if it's a tuple (from v1.4.x format)
            if isinstance(costume, tuple):
                # Reset to v1.1.1 default format
                persistent.chibika_current_costume = blanket_chibi
            
            # Check if it's a list (expected v1.1.1 format)
            elif isinstance(costume, list):
                # Verify it contains valid sprite names (strings that are actual sprite files, not folder names)
                # v1.4.x folder names are: "darling", "cupcake", "cinnamon", "teacup"
                invalid_folders = ["darling", "cupcake", "cinnamon", "teacup"]
                if len(costume) > 0 and costume[0] in invalid_folders:
                    # This is likely corrupted data from a mixed upgrade/downgrade
                    persistent.chibika_current_costume = blanket_chibi
                else:
                    # Verify sprites exist
                    for sprite_name in costume:
                        sprite_path = "Submods/ExtraPlus/submod_assets/sprites/{0}.png".format(sprite_name)
                        if not renpy.loadable(sprite_path):
                            # Sprite doesn't exist, reset to default
                            persistent.chibika_current_costume = blanket_chibi
                            break
            else:
            ...
            # If anything goes wrong, reset to prevent crashes
            persistent.chibika_current_costume = blanket_chibi
        
        # === Migrate accessory layer data ===
        # v1.4.x uses strings like "cat_ears", "0nothing"
        # v1.1.1 uses integers (indices into the accessories list)
        try:
            # Check chibi_accessory_layer_1
            if not isinstance(persistent.chibi_accessory_layer_1, int):
                persistent.chibi_accessory_layer_1 = 0
            elif persistent.chibi_accessory_layer_1 < 0:
                persistent.chibi_accessory_layer_1 = 0
                
            # Check chibi_accessory_layer_2
            if not isinstance(persistent.chibi_accessory_layer_2, int):
                persistent.chibi_accessory_layer_2 = 0
            elif persistent.chibi_accessory_layer_2 < 0:
                persistent.chibi_accessory_layer_2 = 0
        except Exception:
            persistent.chibi_accessory_layer_1 = 0
            persistent.chibi_accessory_layer_2 = 0
...
screen submod_interactions():
    ...
    vbox:
        ...
        textbutton ("Cerrar") action [Hide("submod_interactions"), Jump("close_extraplus")]
        textbutton ("Cita") action [Hide("submod_interactions"), Jump("plus_walk")]
        textbutton ("Jugar") action If(mas_affection._get_aff() >= 30, true=[Hide("submod_interactions"), Jump("plus_minigames")], false=None)
        textbutton ("Adicional") action [Hide("submod_interactions"), Jump("plus_tools")]
...
screen sticker_customization():
    ...
    vbox:
        ...
        textbutton ("Cerrar") style "hkb_button" action [Hide("sticker_customization"), Jump("close_dev_extraplus")]
        textbutton ("Regresar") style "hkb_button" action [Hide("sticker_customization"), Jump("plus_tools")]
    frame:
        padding (50, 10, 0, 20)
        xpos 800
        yanchor 1.0
...
screen boop_revamped():
    ...
    vbox:
        ...
        textbutton ("Cerrar") style "hkb_button" action [Hide("extra_gen_list"), Jump("close_boop_screen")]
        textbutton ("Regresar") style "hkb_button" action [Hide("extra_gen_list"), Jump("return_boop_screen")]
...
screen shell_game_minigame():
    ...
    vbox:
        ...
        yanchor 1.0
        ypos 0.950
        textbutton ("Rendirse") action [Hide("shell_game_minigame"), Jump("shell_game_result")]
...
screen RPS_mg():
    ...
    vbox:
        ...
        yanchor 1.0
        ypos 0.950
        textbutton ("Rendirse") style "hkb_button" action [Hide("RPS_mg"), Jump("rps_quit")]
...
screen extra_gen_list(extra_list, extra_area, others, close=None):
    ...
    if close:
        vbox:
            ...
            textbutton ("Cerrar") style "hkb_button" action [Hide("extra_gen_list"), Jump("close_extraplus")]
...
screen dating_loop(acs, acs_two, ask, finish, label_boop, boop_enable=None):
    ...
    vbox:
        ...
        if store.mas_submod_utils.isSubmodInstalled("Noises Submod"):
            ...
            ypos 595
        textbutton ("Hablar") style "hkb_button" action [Hide("dating_loop"), Jump(ask)]
